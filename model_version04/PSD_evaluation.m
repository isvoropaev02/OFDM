...
full_signal = zeros([512+cp_length,num_of_frames*2]);
for k=1:2:num_of_frames
    ...
    info_frame_td = add_cyclic_prefix(ifft(info_frame, 512).*fr_len, cp_length);
    pilots_frame_td = add_cyclic_prefix(ifft(pilots_frame, 512).*fr_len, cp_length);
...
[psd, freq] = pwelch(full_signal,hamming(floor(L/256)),floor(L/512),L*2^3, delta_f*512); % Hamming window, two-sided psd estimate, 50% overlap 
figure(1)
plot(freq*1e-6 - freq(end)/2*1e-6, fftshift(10*log10(psd)))
